...
<title>DISCOVER rent a car - Ushuaia - Posnet OnLine</title>
<style type="text/css">
img{
border:none;
}
...
if($_GET['ref']=="procesa"){
	include("form2.php");
}elseif($_GET['ref']=="exito"){

	$nroOperacion = $_POST['noperacion'];
	...
	ob_end_clean();
	$nombreFile = ($arrayNroOperacion[0]=='AUTO')?'Reserva_'.date('Ymd'):'PosNet_'.date('Ymd');
	 $dt = date('Y-m-d G:i:s');

	 $_Log = fopen("logs/" . $nombreFile . ".log", "a+") or die("Operation Failed!");

	 fputs($_Log, $dt . $data ."\n");
	if (($arrayNroOperacion[0]=='AUTO')||($arrayNroOperacion[0]=='MANUAL')) {
		include("../conex.php");
		include("../db.php");
		...
		if ($_POST['resultado']=='APROBADA') {
			...
			if ($a6) {
				...
				$laSQL = "SELECT * FROM  cobro_tarjeta_tipos where cobro_tarjeta_posnet_id= '1' AND id_decidir = '".$arrayNroOperacion[5]."'";
