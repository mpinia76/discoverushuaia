...
{
    const CRON_NEXT_DAY_APPOINTMENT     = 1;
    const CRON_FOLLOW_UP_EMAIL          = 2;
    ...
    public static function send( Entities\CustomerAppointment $ca )
    {
        ...
        switch ( $ca->get( 'status' ) ) {
            case Entities\CustomerAppointment::STATUS_PENDING:
                foreach ( array( 'email', 'sms' ) as $gateway ) {
                    $to_client = new Entities\Notification();
                    $to_client->loadBy( array( 'type' => 'client_pending_appointment', 'gateway' => $gateway ) );

                    $to_staff = new Entities\Notification();
                    $to_staff->loadBy( array( 'type' => 'staff_pending_appointment', 'gateway' => $gateway ) );
                ...
                break;
            case Entities\CustomerAppointment::STATUS_APPROVED:
                foreach ( array( 'email', 'sms' ) as $gateway ) {
                    $to_client = new Entities\Notification();
                    $to_client->loadBy( array( 'type' => 'client_approved_appointment', 'gateway' => $gateway ) );

                    $to_staff = new Entities\Notification();
                    $to_staff->loadBy( array( 'type' => 'staff_approved_appointment', 'gateway' => $gateway ) );
            ...
            case Entities\CustomerAppointment::STATUS_CANCELLED:
                foreach ( array( 'email', 'sms' ) as $gateway ) {
                    $to_staff = new Entities\Notification();
					...
					$to_client = new Entities\Notification();
					$to_client->loadBy( array( 'type' => 'client_cancelled_appointment', 'gateway' => $gateway ) );
					if ( $to_client->get( 'active' ) ) {
						// Client time zone offset.
						if ( $ca->get( 'time_zone_offset' ) !== null ) {
							$codes->set( 'appointment_start', Utils\DateTime::applyTimeZoneOffset( $appointment->get( 'start_date' ), $ca->get( 'time_zone_offset' ) ) );
     ...
     *
     * @param int $type  (CRON_NEXT_DAY_APPOINTMENT|CRON_FOLLOW_UP_EMAIL)
