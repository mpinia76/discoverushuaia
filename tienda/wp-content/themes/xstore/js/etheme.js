    ...
    etTheme = {
        ...
        tabs: function() {
            ...
            leftTabs.each(function(){
                ...
                newTitles = currTab.find('.tabs-nav').clone();
            ...
            tabs.each(function(){
                ...
                if( tabs.hasClass('accordion') || tabs.hasClass('left-bar') ) {
                    currTab.find('.tabs-nav').remove();
                    currTab.find('.accordion-title').first().addClass('opened-parent');
                }

                currTab.on('click', '.tab-title, .tab-title-left', function(e){
                    e.preventDefault();
                    var tabId = $(this).attr('id');

                    if(tabOpened(tabId)){
                        closeTab(currTab, tabId, false);
                    }else{
                        closeAllTabs(currTab);
                        setTimeout(function() {
                            openTab(currTab, tabId);
                        }, time );
                    }
                });
            });
            ...
            var openTab = function(tabs, id) {
                if( tabs.hasClass('accordion') || ($(window).width() < 767 && ! tabs.hasClass('products-tabs')) ) {
                    $('#content_'+id).slideDown(time); // Fix it
